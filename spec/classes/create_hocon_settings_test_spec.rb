...
describe 'create_hocon_settings_test' do
  ...
  it do
    should contain_hocon_setting('/tmp/foo.conf section1.setting1').with({
      :ensure  => 'present',
  ...
  it do
    should contain_hocon_setting('/tmp/foo.conf section2.setting2').with({
      :ensure  => 'present',
  ...
  it do
    should contain_hocon_setting('/tmp/foo.conf foo.bar.baz').with({
      :ensure  => 'present',
      :setting => 'foo.bar.baz',
      :value   => 'val3',
      :path    => '/tmp/foo.conf',
